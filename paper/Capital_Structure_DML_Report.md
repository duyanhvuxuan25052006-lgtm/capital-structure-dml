...
*Ghi chú: * p < 0,10; ** p < 0,05; *** p < 0,01; "không có" = không có ý nghĩa thống kê (not significant). Ba loại R² trong bảng đo lường những đại lượng khác nhau (Full R² cho OLS đo độ khớp toàn phần, Within R² cho FE đo độ khớp biến động thực thể, và Nuisance R² cho DML đo độ khớp của các hàm phi can thiệp), do đó không nên so sánh trực tiếp độ lớn giữa các dòng.*
...
- DML, nhờ học đúng dạng hàm phi tuyến của $g(X)$ và $m(X)$ bằng Random Forest, giúp giảm thiểu thiên lệch dạng hàm — cho thấy bằng chứng thực nghiệm về tác động tích cực và có ý nghĩa thống kê của đòn bẩy tài sản lên ROA trong phạm vi mẫu nghiên cứu.

**④ Kiểm định độ ổn định của hệ số tác động (Sensitivity Analysis qua 30 Seeds)**

Để đảm bảo hệ số tác động tích cực của mô hình Random Forest trong DML là thực chất và không phụ thuộc vào việc chọn hạt ngẫu nhiên (random seed) may mắn, chúng tôi tiến hành kiểm định độ ổn định hệ thống. Mô hình DML-RandomForest được chạy lặp lại 30 lần với các hạt ngẫu nhiên chạy từ 1 đến 30. 

Kết quả cho thấy sự ổn định tuyệt đối của ước lượng:
- **Tỷ lệ có ý nghĩa thống kê:** 100% số lần chạy (30/30 lần) đều cho ra hệ số $\theta$ dương và có ý nghĩa thống kê ở mức 5% ($p < 0,05$).
- **Độ biến động của hệ số $\theta$:** Giá trị $\theta$ dao động trong khoảng cực hẹp từ $+0,0306$ đến $+0,0373$ với giá trị trung bình là $+0,0340$ và độ lệch chuẩn cực nhỏ ($SD = 0,0017$).
- **Độ biến động của p-value:** Nằm trong khoảng từ $0,0106$ đến $0,0393$, luôn nhỏ hơn mức ý nghĩa 5%.

Kết quả kiểm định độ nhạy này (chi tiết được lưu tại file `results/rf_stability_check.csv`) khẳng định mạnh mẽ tính vững chắc của mối quan hệ nhân quả tích cực từ đòn bẩy tài sản sang ROA.
...
1. **Đối chiếu với Dang & Do (2021) (System GMM):**
   * *Sự phân kỳ:* Dang & Do (2021) tìm thấy tác động không có ý nghĩa thống kê của đòn bẩy tài sản lên giá trị doanh nghiệp ở mẫu gộp toàn bộ nền kinh tế. Kết quả của chúng tôi trong các mô hình tuyến tính (Entity FE, Two-way FE, DML-Lasso) cũng cho ra kết quả trung tính tương tự ($\theta \approx -0,0066$ đến $-0,0095$, không có ý nghĩa thống kê). Tuy nhiên, khi sử dụng mô hình phi tuyến DML-Random Forest trên nhóm Blue-chips, kết quả cho thấy bằng chứng về tác động dương có ý nghĩa thống kê ($\theta = 0,0313$, $p < 0,05$).
   * *Ý nghĩa phương pháp luận:* Kết quả này gợi ý khả năng tồn tại thiên lệch dạng hàm (functional form bias) trong các nghiên cứu tuyến tính trước đây. Khi kiểm soát các tương tác phi tuyến phức tạp của các biến kiểm soát tài chính bằng thuật toán học máy trong phạm vi nhóm Blue-chips, mối liên hệ tích cực giữa cấu trúc vốn và hiệu quả hoạt động mới được phản ánh rõ nét hơn, thay vì bị triệt tiêu bởi giả định tuyến tính.

2. **Đối chiếu với Nguyen et al. (2023) (Pooled OLS & FGLS):**
   * *Sự phân kỳ:* Nguyen et al. (2023) phát hiện nợ dài hạn tác động âm ý nghĩa lên hiệu quả hoạt động của 300 doanh nghiệp niêm yết. Trong khi đó, mô hình phi tuyến DML-Random Forest của chúng tôi trên mẫu 50 doanh nghiệp Blue-chips lại ghi nhận tác động tích cực.
   * *Giải thích nguyên nhân:*
     * **Sự khác biệt về đối tượng nghiên cứu:** Mẫu của Nguyen et al. (2023) gồm 300 doanh nghiệp niêm yết với nhiều quy mô khác nhau (bao gồm cả các doanh nghiệp vừa và nhỏ). Đối với các doanh nghiệp quy mô trung bình và nhỏ, chi phí sử dụng vốn thường cao hơn và năng lực quản lý dòng tiền có thể chưa đạt mức tối ưu, khiến việc vay nợ dễ dẫn đến chi phí kiệt quệ tài chính (tác động âm). Ngược lại, nghiên cứu của chúng tôi chỉ tập trung vào **nhóm 50 Blue-chips lớn nhất**, những doanh nghiệp có khả năng thương lượng lãi suất tốt và có thể sử dụng nợ hiệu quả hơn như một công cụ kỷ luật quản lý (Agency Theory). Sự phân kỳ này do đó phản ánh đặc thù của từng nhóm đối tượng nghiên cứu, chứ không phủ nhận tính đúng đắn của các nghiên cứu trước trên mẫu rộng.
     * **Vai trò của thiên lệch dạng hàm:** Khi chúng tôi chạy mô hình OLS đơn giản không kiểm soát Fixed Effects (Mục 5.1), hệ số ước lượng đòn bẩy cũng cho kết quả âm lớn ($\theta = -0,2016$, $p < 0,0001$), tương tự như xu hướng của Nguyen et al. (2023). Tuy nhiên, việc kiểm soát Fixed Effects và xử lý các ảnh hưởng phi tuyến bằng DML giúp giảm thiểu chệch dạng hàm, từ đó làm bộc lộ tác động tích cực của đòn bẩy đối với nhóm doanh nghiệp lớn phi tài chính trong mẫu.
...
| :--- | :--- | :--- |
| **Độ nhạy đối với lựa chọn random seed:** Kết quả DML-RandomForest có thể nhạy với lựa chọn random seed do bản chất ngẫu nhiên của thuật toán | Thấp | Đã chạy kiểm tra độ ổn định trên 30 seeds khác nhau (seeds 1-30) ở Mục 5.2 và xác nhận tỷ lệ có ý nghĩa thống kê đạt 100% |
